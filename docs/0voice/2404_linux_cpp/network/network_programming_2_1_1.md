...
Linux 中网络通信的本质是借助内核，使用内核提供的伪文件机制，将套接字与文件描述符绑定，使所有的操作都是通过文件描述符(可阅读[文件 I/O](../../../linux/linux_system_programming/file_io.md) 详细了解文件描述符)。我们通过一个简单的服务器和客户端程序实例简单理解：
...
理解服务端程序的流程：当程序启动后，此时程序就到了流程中的 `accept` 函数调用处，等待客户端的连接。一旦有一个客户端连接成功，程序就到了流程中的 `recv` 这里。然而这个客户端的流程还没有结束(客户端的流程结束是此客户端断开)，我们又连接了其他的客户端，并且其他客户端向服务器端发送数据，服务器端此时是无法处理其他客户端发来的数据。因为第一个客户端的数据处理流程还没有跑完(客户端还没有断开)，其他的客户端的数据处理流程就不可能执行。如果我们希望每个客户端能够独立发送数据，就需要用到多进程或多线程，一旦一个客户端连接后就立即创建一个进程或线程进行数据收发，这样就不会因为代码逻辑而阻塞(可阅读[进程控制](../../../linux/linux_system_programming/process_control.md)或[线程](../../../linux/linux_system_programming/threads.md)理解)。代码示例如下：
