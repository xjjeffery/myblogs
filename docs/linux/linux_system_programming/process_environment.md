...
!!! info
    ...
    该节对应 APUE 第 7 章 —— 进程环境，相关详细内容请阅读本章节或查阅 man 手册。
...
程序正常终止有三个函数，分别是 `exit`、`_exit` 以及 `_EXIT`，它们三个之间有说明联系和区别呢。
...
!!! question "怎么区分使用 `exit` 和 `_exit` 或 `_Exit`？"
...
`atexit` 的参数是一个函数地址，也就是函数指针，该函数没有参数也没有返回值。`atexit` 调用这些函数的顺序与登记时候的顺序相反，同一个函数如果登记多次，也会被调用多次(有点类似 C++ 中的析构函数)。

!!! note

    可以在任何地方注册终止函数，但是都是在程序终止的时候被调用。
...
环境变量的含义：程序(操作系统命令和应用程序)的执行都需要运行环境，这个环境时由多个环境变量组成的。
...
!!! question "这些函数在修改环境表时是如何进行操作的？"
    ...
    - 如果修改一个现有的 `name`：
        - 如果新 `value` 的长度少于或等于现有 `value` 的长度，则只要将心字符串复制到原字符串所用的空间中；
        - 如果新 `value` 的长度大于原长度，则必须调用 `malloc` 为新字符串分配空间，然后将新字符串复制到该空间中，接着使环境表中针对针对 `name` 的指针指向新分配区。
...
在 C 中，`goto` 语句是不能跨越函数的，而执行这种类型跳转功能的函数是 `setjmp` 和 `longjmp`，这两个函数对于处理发生在很深层嵌套函数调用中的出错情况非常有用。
...
!!! question "通过 `longjmp` 返回以后，返回的变量值是恢复到之前的值还是后续函数调用中修改的值？"
    ...
    after longjmp
    globval = 100, lval = 92, regival = 3, volaval = 94, staval = 
...
RLIMIT_CPU        // 最大允许的 CPU 使用时间，秒为单位。当进程达到软限制，内核将给其发送 SIGXCPU 信号
                  // 这一信号的默认行为是终止进程的执行。然而，可以捕捉信号，处理句柄可将控制返回给主程序
                  // 如果进程继续耗费 CPU 时间，核心会以每秒一次的频率给其发送 SIGXCPU 信号，直到达到硬限制
                  // 那时将给进程发送 SIGKILL 信号终止其执行
